...
function TransferCryptoView({ hasUser, isVerified, balance, transfers, suppliers, supplierPayments, enabledNetworks, loading, onSubmit, onCreateSupplier, onSupplierPayment, onContinue, onRefresh }: { hasUser: boolean; isVerified: boolean; balance: BalanceSummary | null; transfers: BalanceTransferRecord[]; suppliers: SupplierRecord[]; supplierPayments: SupplierPaymentRecord[]; enabledNetworks: NetworkControl[]; loading: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCreateSupplier: (event: FormEvent<HTMLFormElement>) => void; onSupplierPayment: (event: FormEvent<HTMLFormElement>) => void; onContinue: () => void; onRefresh: () => Promise<void> }) {
  ...
  return <section className="app-page transfer-premium"><PageHero title="Transfer & pay" subtitle="Move settled USDC from your Bridge-backed Sivan account. Sivan does not hold a live USD fiat balance for you." action={<button className="primary-btn small" onClick={() => void onRefresh()}>Refresh USDC balance</button>} />
    <div className="transfer-route-grid">
      <article className="transfer-route-card active"><span>⇆</span><div><strong>Send crypto</strong><small>Transfer settled USDC to your own wallet on a supported network.</small></div><Badge status="active">Available</Badge></article>
      <article className="transfer-route-card"><span>▭</span><div><strong>Pay supplier / cross-border</strong><small>Crypto-to-fiat payout to a saved supplier bank account after compliance checks.</small></div><Badge status="pending">Ops review</Badge></article>
      <article className="transfer-route-card"><span>◇</span><div><strong>Send to Sivan user</strong><small>Future Bridge-wallet transfer between approved Sivan users.</small></div><Badge status="pending">Coming soon</Badge></article>
    </div>
