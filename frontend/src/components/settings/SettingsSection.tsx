...
function ProfileSettingsPanel({ api, user, isVerified, identityStatus, pairingCode, pairingExpiresAt, timeNow, loading, onUserUpdated, onStartWhatsappLink, onCancelWhatsappLink, onUnlinkWhatsapp, onRefreshIdentity, telegramPairingCode, telegramPairingExpiresAt, onStartTelegramLink, onCancelTelegramLink, onUnlinkTelegram }: { api: <T>(path: string, options?: RequestInit) => Promise<T>; user: UserRecord | null; isVerified: boolean; identityStatus: IdentityStatus | null; pairingCode: string; pairingExpiresAt: string; timeNow: number; loading: boolean; onUserUpdated: (user: UserRecord) => void; onStartWhatsappLink: () => void; onCancelWhatsappLink: () => void; onUnlinkWhatsapp: () => void; onRefreshIdentity: () => Promise<void>; telegramPairingCode: string; telegramPairingExpiresAt: string; onStartTelegramLink: () => void; onCancelTelegramLink: () => void; onUnlinkTelegram: () => void; }) {
  ...
  async function uploadAvatar(file?: File) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setAvatarError('Profile photo must be 5MB or less.');
      return;
    }
    setUploading(true);
    try {
  ...
  async function executeRemoveAvatar() {
    ...
    try {
      ...
      setAvatarError(error instanceof Error ? error.message : 'Could not remove profile photo.');
    } finally {
      setUploading(false);
    }
  }
...
function IdentityLinkCard({ identityStatus, pairingCode, pairingExpiresAt, timeNow, loading, onStart, onCancel, onUnlink, onRefresh }: { identityStatus: IdentityStatus | null; pairingCode: string; pairingExpiresAt: string; timeNow: number; loading: boolean; onStart: () => void; onCancel: () => void; onUnlink: () => void; onRefresh: () => Promise<void> }) {
  // channels.whatsapp is the precise field; the top-level pair is the legacy
  // alias of exactly the same record, kept as a fallback for an older server.
  const link = identityStatus?.channels?.whatsapp?.link ?? identityStatus?.link;
  const pending = identityStatus?.channels?.whatsapp?.pendingPairing ?? identityStatus?.pendingPairing;
...
/**
 * The Telegram twin of IdentityLinkCard.
 *
 * Reads channels.telegram rather than the legacy top-level fields, which are
 * WhatsApp's. Deliberately has NO deeplink: t.me/<bot>?text= does not reliably
 * prefill a message, so offering one would produce a button that silently does
 * the wrong thing. The user copies the code and types /link instead, which is
 * what the bot documents.
 */
function TelegramLinkCard({ channel, pairingCode, pairingExpiresAt, timeNow, loading, onStart, onCancel, onUnlink, onRefresh }: { channel: IdentityChannelStatus | undefined; pairingCode: string; pairingExpiresAt: string; timeNow: number; loading: boolean; onStart: () => void; onCancel: () => void; onUnlink: () => void; onRefresh: () => Promise<void> }) {
  const link = channel?.link;
  const pending = channel?.pendingPairing;
  const expiresAt = pairingExpiresAt || pending?.expiresAt || '';
  const secondsLeft = expiresAt ? Math.max(0, Math.ceil((new Date(expiresAt).getTime() - timeNow) / 1000)) : 0;
  const copyPairingCode = async () => {
    if (!pairingCode) return;
  ...
  return <div className="identity-link-card"><div><p className="eyebrow">Sivan unified identity</p><h3>Linked Telegram account</h3><p className="muted">Link your Telegram account so the Sivan bot and your web dashboard use one customer profile.</p></div>{link ? <div className="identity-link-status linked"><span>Linked</span><strong>{link.telegramUsername ? `@${link.telegramUsername}` : `Telegram ID ${link.telegramUserId || ''}`}</strong><small>Linked {link.linkedAt ? new Date(link.linkedAt).toLocaleString() : 'recently'}</small><div className="identity-link-actions"><button type="button" className="ghost-btn small" disabled={loading} onClick={() => void onRefresh()}>Refresh status</button><button type="button" className="ghost-btn small" disabled={loading} onClick={onUnlink}>Unlink</button></div></div> : pending ? <div className="identity-link-status pending">{/* A RELOAD CANNOT RECOVER THE CODE - the server never returns it
        ...
        box that looks broken. Regenerate is the way out. */}<span>Pairing code active</span><strong>{pairingCode || 'Code generated'}</strong><small>{secondsLeft ? `Expires in ${Math.floor(secondsLeft / 60)}m ${secondsLeft % 60}s` : `Expires ${new Date(pending.expiresAt).toLocaleString()}`}</small>{!pairingCode && <small>Your code is no longer displayable after a page reload. Cancel it and generate a new one.</small>}<div className="identity-link-actions"><button type="button" className="ghost-btn small" disabled={!pairingCode || loading} onClick={copyPairingCode}>Copy code</button>{pairingCode && <a className="secondary-btn small" href={`https://t.me/${botUsername}?start=${pairingCode}`} target="_blank" rel="noopener noreferrer">Open in Telegram ↗</a>}<button type="button" className="ghost-btn small" disabled={loading} onClick={() => void onRefresh()}>Refresh</button><button type="button" className="ghost-btn small" disabled={loading} onClick={onCancel}>Cancel code</button></div><small>In Telegram, send <code>/link</code> followed by this code to the Sivan bot, or tap <strong>Open in Telegram</strong> above.</small></div> : <div className="identity-link-status"><span>Not linked</span><strong>Connect your Telegram account</strong><small>Generate a code, then send /link with it to the Sivan bot on Telegram.</small><div className="identity-link-actions"><button type="button" className="secondary-btn" disabled={loading} onClick={onStart}>Generate pairing code</button><button type="button" className="ghost-btn small" disabled={loading} onClick={() => void onRefresh()}>Refresh status</button></div></div>}</div>;
}
