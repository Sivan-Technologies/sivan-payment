...
Every endpoint comes from the environment. There is no fallback URL
...
| npm run test:cashout | MoneyGram quote, receiver validation, commit (REST API) |
...
Dual-Track MoneyGram Testing:
- Track 1 (Stellar Anchor SEP-10 / SEP-24): Interactive deposit and withdrawal webview against extmgxanchor.moneygram.com / extstellar.moneygram.com.
- Track 2 (Developer REST API): Direct server-to-server quote, receiver validation, and payout commit against sandboxapi.moneygram.com.
...
The colon separator is load-bearing. Without it ("ab","c14") and ("abc","14")
...
Stellar consensus timing: Stellar closes ledgers in roughly five
