...
API_PORT="${API_PORT:-4730}"
APP_PORT="${APP_PORT:-4174}"
DB=".data/e2e-supfee.json"
...
npx tsx src/server.ts > /tmp/supfee-api.log 2>&1 &
API_PID=$!
...
if ! kill -0 "$API_PID" 2>/dev/null; then
  ...
  tail -20 /tmp/supfee-api.log >&2
  exit 1
fi

(cd frontend && VITE_API_BASE_URL="http://127.0.0.1:$API_PORT" ./node_modules/.bin/vite build --outDir dist >/tmp/supfee-build.log 2>&1)

APP_PORT="$APP_PORT" python3 e2e/serve-dist.py > /tmp/supfee-web.log 2>&1 &
WEB_PID=$!
sleep 2

API="http://127.0.0.1:$API_PORT" APP="http://127.0.0.1:$APP_PORT" \
  node e2e/supplier-fee-journey.mjs
