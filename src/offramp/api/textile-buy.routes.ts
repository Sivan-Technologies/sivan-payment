...
export async function textileBuyRoutes(app: FastifyInstance) {
  async function upstream(path: string, body?: unknown, claim?: string) {
    ...
    const response = await fetch(`${base}/ramp${path}`, {
    ...
    if (!response.ok) throw Object.assign(new Error(data.error?.message || 'Textile request failed'), { statusCode: response.status });
    return data;
  }
  ...
  app.post('/api/v1/buy-cngn/orders', async (request, reply) => {
    ...
    if (review.kyc?.state !== 'verified' || review.kyc?.canDeposit !== true) {
      return reply.code(422).send({ error: 'Identity verification is required before buying.', kyc: review.kyc });
    }
    return upstream('/transfers', { ...body, side: 'buy', fiat: 'NGN', token: 'CNGN' });
  });
