...
function downloadCsv(fileName: string, rows: Record<string, unknown>[]) {
  if (!rows.length) return downloadTextFile(fileName, '', 'text/csv');
  const keys = Array.from(rows.reduce<Set<string>>((set, row) => { Object.keys(row).forEach((key) => set.add(key)); return set; }, new Set<string>()));
  const csv = [keys.join(','), ...rows.map((row) => keys.map((key) => csvCell(row[key])).join(','))].join('\n');
  downloadTextFile(fileName, csv, 'text/csv');
}

function openAdminView(view: AdminViewKey) {
  window.dispatchEvent(new CustomEvent('sivan-admin-open-view', { detail: view }));
}
...
export default function App() {
  ...
  useEffect(() => {
    const openView = (event: Event) => {
      const detail = (event as CustomEvent<AdminViewKey>).detail;
      if (detail) setView(detail);
    };
    window.addEventListener('sivan-admin-open-view', openView);
    return () => window.removeEventListener('sivan-admin-open-view', openView);
  }, []);
        ...
        <header className="adm-topbar">
          ...
          <div className="adm-top-right">
            ...
            <div className="adm-profile-wrap">
              ...
              {profileOpen && <div className="adm-profile-dd" onMouseLeave={() => setProfileOpen(false)}><div className="adm-dd-head"><div className="adm-avatar lg grad-bg">{adminInitials}</div><div><div className="adm-dd-name">Sivan Admin</div><div className="adm-dd-mail">admin@sivantech.online</div><span className="adm-dd-role">Super Admin</span></div></div><button className="adm-dd-item" onClick={() => setView('settings')}>⚙ Runtime settings</button><button className="adm-dd-item" onClick={refreshAdmin}>⟳ Refresh data</button><button className="adm-dd-item danger" onClick={() => notify('Your existing admin login/RBAC will be connected here later.')}>Admin auth moves here later</button></div>}
            </div>
          </div>
        </header>
...
function Users({ users, api, notify }: { users: AdminUser[]; api: <T>(path: string, options?: RequestInit) => Promise<T>; notify: (message: string, type?: 'success' | 'error') => void }) {
  ...
  return <section className="adm-page"><PageHead title="Users" sub="Manage, verify and monitor every customer account." actions={<><button className="btn-ghost" onClick={() => downloadCsv('sivan-users-loaded.csv', filtered as any)}>Export</button><button className="btn-primary" onClick={() => window.open('https://app.sivantech.online/signup', '_blank')}>New user</button></>} /><div className="stat-strip"><div className="ss-item"><strong>{users.length}</strong><span>Total users</span></div><div className="ss-item ok"><strong>{users.filter((u) => u.customer?.kycStatus === 'kyc_approved').length}</strong><span>Verified</span></div><div className="ss-item blue"><strong>{users.filter((u) => u.customer?.kycStatus && u.customer.kycStatus !== 'kyc_approved').length}</strong><span>KYC pending</span></div><div className="ss-item gold"><strong>{users.reduce((sum, u) => sum + u.withdrawalCount, 0)}</strong><span>Withdrawals</span></div></div><section className="panel-grid two"><article className="adm-card"><FilterBar><SearchBar value={query} onChange={setQuery} placeholder="Search by name, email or ID..." /><div className="chips">{[['all','All'],['verified','Verified'],['pending','KYC pending'],['kyc_rejected','Rejected']].map(([value,label]) => <button key={value} className={`chip ${filter===value ? 'active':''}`} onClick={() => setFilter(value)}>{label}</button>)}</div><button className="btn-ghost-sm" onClick={() => notify('Filters are already visible above the table.')}>Filters</button><button className="btn-ghost-sm" onClick={() => downloadCsv('sivan-users-loaded.csv', filtered as any)}>Export CSV</button></FilterBar>{!filtered.length ? <Empty>No users found.</Empty> : <div className="adm-table-wrap"><table className="adm-table"><thead><tr><th>User</th><th>Email</th><th>KYC</th><th>Type</th><th>Onboarding cost</th><th>Accounts</th><th>Withdrawals</th></tr></thead><tbody>{filtered.map((user) => <tr className="clickable" key={user.id} onClick={() => openUser(user.id)}><td><div className="us-cell"><span className="us-av grad-bg">{(user.fullName || user.email || 'SA').slice(0,2).toUpperCase()}</span><div><div className="us-name">{user.fullName || 'Sivan user'}</div><div className="us-mail text-faint mono">{user.id}</div></div></div></td><td>{user.email}</td><td><Badge value={user.customer?.kycStatus || 'not_started'} /></td><td>{user.customer?.customerType || '—'}</td><td>{user.customer?.onboardingCostUsd ? `$${user.customer.onboardingCostUsd}` : '—'}</td><td>{user.externalAccountCount}</td><td>{user.withdrawalCount}</td></tr>)}</tbody></table></div>}</article><DetailDrawer title="Customer profile" data={selected} empty="Select a user to view KYC, legal acceptance, bank accounts, transactions, support, risk flags, notes, and audit timeline." /></section></section>;
}
...
function Settings({ apiBase, platformSettings, teamMembers, apiKeys, api, notify, onUpdated, adminApiKey }: { apiBase: string; platformSettings: any; teamMembers: any[]; apiKeys: any[]; api: <T>(path: string, options?: RequestInit) => Promise<T>; notify: (message: string, type?: 'success' | 'error') => void; onUpdated: () => Promise<void>; adminApiKey: string }) {
  ...
  async function inviteTeammate() {
    ...
    const role = window.prompt('Role (Operator, Compliance, Finance, Support, Auditor)') || 'Operator';
    if (!name || !email) return;
    try {
      await api('/api/admin/settings/team/invite', { method: 'POST', body: JSON.stringify({ name, email, role, invitedBy: 'admin_api_key', reason: 'Invite from admin settings' }) });
      notify('Team invitation recorded. Connect email delivery when admin auth is moved in.');
      await onUpdated();
    } catch (error) { notify(errorMessage(error), 'error'); }
  }
